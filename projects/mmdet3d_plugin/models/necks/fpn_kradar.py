...
class FPN_KRADAR(FPN_TRANS):
    ...
    def __init__(self,
                 ...
                 cam_types=['FRONT','FRONT_RIGHT','FRONT_LEFT',
                                'BACK','BACK_LEFT','BACK_RIGHT'],
                 fpn_cfg=None):
        super(FPN_KRADAR, self).__init__(
            grid_res=grid_res,
            pc_range=pc_range,
    ...
    def forward(self, feature_list, img_metas, radar_list, gt_bboxes_3d, gt_labels_3d):
        ...
        Trv2c = [img_meta['Trv2c'] for img_meta in img_metas]
        ...
        for lvl in range(self.num_levels):
            ...
            bev_feat, mask = self._interpolate_by_image_column(
                feature_size, scale, cam_type, ret_polar_ray_list[lvl], lidar2img, Trv2c, lvl)
    ...
    def _interpolate_by_image_column(self, feature_size, scale, cam_type, polar_ray, lidar2img, Trv2c, lvl):
        ...
        corners = torch.cat([xx.unsqueeze(-1).cos()*yy.unsqueeze(-1),
                            xx.unsqueeze(-1).sin()*yy.unsqueeze(-1), zz.unsqueeze(-1)], dim=-1)
        
        # Project to image plane, calib is same for all frames
        dimx, dimy, dimz = corners.shape[:3]
        corners = corners.reshape(-1, 3)
        corners_homo = torch.cat([corners, torch.ones(corners.shape[0], 1)], dim=-1)
        img_corners = (torch.from_numpy(lidar2img[0]).float() @ corners_homo.t()).t()
        img_corners = img_corners[..., :2] / img_corners[..., 2, None]
        img_corners = img_corners.reshape(dimx, dimy, dimz, -1)
        ...
        rot_mat = Trv2c[0][:3, :3]
        cam_coords = -np.linalg.inv(rot_mat) @ Trv2c[0][:3, 3]
        ...
        # for visualization; TODO: remove this part
        
        # feat_map = interpolated_feat.abs().sum(dim=1)[0]
        # feat_map = feat_map / feat_map.max()
        # plt.imshow(feat_map.cpu().detach().numpy())
        # plt.title(label='interpolated_feat map')
        # plt.savefig('vis/interpolated_feat_map.png')

        # feat_map = visible.abs()[0]
        # nonzero_index = feat_map.nonzero()[:, 1]
        # min_idx, max_idx = nonzero_index.min(), nonzero_index.max()
        # print(f"Valid anlge range: {min_idx/feat_map.shape[-1]*60-30} ~ {max_idx/feat_map.shape[-1]*60-30}")
        # plt.imshow(feat_map.cpu().detach().numpy())
        # plt.title(label='visible map')
        # plt.savefig('vis/visible_map.png')
