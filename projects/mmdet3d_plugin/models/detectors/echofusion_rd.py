...
class EchoFusion_RD(PolarFormer):
    """PolarFormer."""
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(EchoFusion_RD, self).__init__(use_grid_mask, pts_voxel_layer, pts_voxel_encoder,
    ...
    def extract_radar_feat(self, radars_rt):
        
        # Unwrap TX information
        B, R, D, _ = radars_rt.shape
        if self.use_log:
            radars_rt[:, :, :, ::2] = torch.log(radars_rt[:, :, :, ::2] + self.epsillon)
        doppler_indexes = []
        ...
        decoded_rt = radars_rt[:,:,doppler_indexes,:].reshape(B, R, D, -1)  # [B, R, D, A*2]

        radars_rt = decoded_rt.permute(0, 3, 1, 2).contiguous()  # [B, A*2, R, D]
        radars_rt = self.radar_embedder(radars_rt)  # [B, C, R, D]
    ...
    def extract_feat(self, img, img_metas, radars_rt, gt_bboxes_3d, gt_labels_3d):
        ...
        for i in range(self.num_levels):
            ...
            radar_x_range = torch.arange(0., float(D), 1., device=feat.device)
            ...
            radar_pos = self.pos_encoding(radar_x_range, radar_y_range) # [B, C, R, D]
            ...
            radar_rows = radar_rows.permute(3, 0, 2, 1).flatten(1, 2) # [D, B*R, C]
    ...
    def forward_train(self,
        ...
        Args:
            ...
            radars_rt (torch.Tensor optional): Radar data of each sample with shape
    ...
    def forward_test(self, img_metas, img=None, radars_rt=None, **kwargs):
        for var, name in [(img_metas, 'img_metas')]:
            if not isinstance(var, list):
                raise TypeError('{} must be a list, but got {}'.format(
                    name, type(var)))
        img = [img] if img is None else img
        radars_rt = [radars_rt] if radars_rt is None else radars_rt
        return self.simple_test(img_metas[0], img[0], radars_rt[0], **kwargs)
        # if num_augs == 1:
        #     img = [img] if img is None else img
    ...
    def simple_test(self, img_metas, img=None, radars_rt=None, rescale=False,gt_bboxes_3d=None, gt_labels_3d=None):
