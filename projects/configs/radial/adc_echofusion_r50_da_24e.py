# lr means lr for range embedder becomes 1/10
_base_ = [
...
train_pipeline = [
    ...
    dict(type='LoadRadarsFromFile', coord_type='LIDAR', modality_type='ADC', convert_complex=False, radar_dim=[512, 256, 16]),
    ...
    dict(type='Collect3D', keys=['gt_bboxes_3d', 'gt_labels_3d', 'img', 'radars_adc'],
    ...
    dict(
        ...
        transforms=[
            dict(
                ...
                with_label=False),
            dict(type='Collect3D', keys=['img', 'radars_adc'],
                 meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                            'cam2lidar', 'cam_intrinsic',
                            'depth2img', 'cam2img', 'pad_shape',
                            'scale_factor', 'flip', 'pcd_horizontal_flip',
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            'range_embedder': dict(lr_mult=0.1),
